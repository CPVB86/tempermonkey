...
// @downloadURL  https://raw.githubusercontent.com/CPVB86/tempermonkey/main/GG/toolbox/adapters/gg-gereserveerd.user.js
// @grant        none
// ==/UserScript==
...
(function () {
    ...
    function parseReservedOrders(doc) {
        ...
        for (const row of doc.querySelectorAll("#reservedForTable tbody tr")) {
            ...
            if (url.origin !== location.origin || !url.pathname.startsWith('/orders/view/')) continue;
    ...
    function loadRenderedProductPage(productUuid) {
        return new Promise((resolve, reject) => {
            if (!allowed()) return reject(new Error('Geen toegang via de core'));
            ...
            iframe.addEventListener("load", () => {
                ...
                pollTimer = setInterval(() => {
                    try {
                        if (!allowed()) return finish(reject, new Error('Geen toegang via de core'));
                        ...
                        if (table && lengthSelect && !requestedLargestPage) {
                            const values = [...lengthSelect.options]
                                .map((option) => Number(option.value))
                                .filter(Number.isFinite)
                                .sort((a, b) => b - a);
                            const largestValue = values.includes(-1) ? -1 : values[0];
    ...
    async function processRow(row) {
        ...
        if (!allowed() || !productUuid || !/^[a-f0-9-]+$/i.test(productUuid)) return;
        ...
        try {
            const orders = await fetchReservedOrders(productUuid);
            if (allowed() && row.isConnected && row.dataset.productUuid === productUuid) showOrders(row, orders);
        } catch (error) {
            console.warn(SCRIPT_NAME, `Controle mislukt voor ${productUuid}`, error);
            if (allowed() && row.isConnected && row.dataset.productUuid === productUuid) showError(row);
        }
    }
    ...
    function processRows(root = document) {
        if (!allowed()) {
            ...
            document.querySelectorAll('[data-gg-reserved-orders-uuid]').forEach(row => delete row.dataset.ggReservedOrdersUuid);
            cache.clear();
            return;
        }

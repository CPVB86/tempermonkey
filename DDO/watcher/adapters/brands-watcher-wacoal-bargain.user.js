// ==UserScript==
// @name         DDO | Brands Watcher - Wacoal, Freya, Fantasie & Elomi
// @version      0.3.2
// @description  Laadt Wacoal, Freya, Fantasie en Elomi uit DDO en vergelijkt alle varianten met de gezamenlijke kortingsbron.
...
// @updateURL    https://raw.githubusercontent.com/CPVB86/tempermonkey/main/DDO/watcher/adapters/brands-watcher-wacoal-bargain.user.js?v=0.3.2
// @downloadURL  https://raw.githubusercontent.com/CPVB86/tempermonkey/main/DDO/watcher/adapters/brands-watcher-wacoal-bargain.user.js?v=0.3.2
...
(function () {
  ...
  const log = (...args) => console.log("[BRANDS-WATCHER][WACOAL-GROUP]", ...args);
  let products = [];
  ...
  function readProducts(buffer, brand = BRAND) {
    ...
    if (!groups.size) {
      ...
      throw new Error(`Geen ${brand}-producten herkend. Gevonden kolommen: ${headers || "geen"}`);
    }
  ...
  async function watcherInit() {
    ...
    api.registerAdapter({
      id: "brands-watcher-wacoal-group",
      name: "DDO | Brands Watcher - Wacoal, Freya, Fantasie & Elomi",
      version: VERSION,
      downloadURL: "https://raw.githubusercontent.com/CPVB86/tempermonkey/main/DDO/watcher/adapters/brands-watcher-wacoal-bargain.user.js"
    });
    ...
    page.addEventListener("brands-watcher:load-ddo", async (event) => {
      try {
        ...
        console.error("[BRANDS-WATCHER][WACOAL-GROUP]", error);
    ...
    page.addEventListener("brands-watcher:inventory-loaded", (event) => {
      ...
      products = importedProducts
        ...
        .map((product) => ({ ...product, supplierUrl: SHEET_URL, warningMode: "missed-discount-only" }));
    });
    ...
    page.addEventListener("brands-watcher:retry", async (event) => {
      ...
      try {
        api.progress(`${product.brand} opnieuw controleren: ${productId}`);
    ...
    page.addEventListener("brands-watcher:start", async (event) => {
      ...
      try {
        ...
        if (!openProducts.length) {
          api.complete(`Geen openstaande ${selectedBrand}-producten om te controleren.`);
          return;
        }
        ...
        for (let index = 0; index < openProducts.length; index++) {
          ...
          api.progress(`${selectedBrand} controleren: ${index + 1}/${openProducts.length} | ${product.productId || product.sku}`);

// ==UserScript==
// @name         DDO | Brands Watcher - Chantelle & Femilet
// @version      0.23.1
// @description  Laadt Chantelle en Femilet uit DDO en toont ontbrekende leveranciersmaten in de Brands Watcher.
...
// @updateURL    https://raw.githubusercontent.com/CPVB86/tempermonkey/main/DDO/watcher/adapters/brands-watcher-chantelle.user.js?v=0.23.1
// @downloadURL  https://raw.githubusercontent.com/CPVB86/tempermonkey/main/DDO/watcher/adapters/brands-watcher-chantelle.user.js?v=0.23.1
...
(function () {
  ...
  const DEFAULT_EXPORT_BRAND_ID = new URLSearchParams(location.search).get("brandId") || "196";
  const SUPPORTED_BRANDS = new Set([
    "Chantelle", "Chantelle Nachtmode", "Chantelle Badmode", "Chantelle EasyFeel",
    "Chantelle Pulp", "Chantelle X", "Femilet Badmode", "Femilet Nachtmode"
  ]);
  ...
  function exportUrl(brandId = DEFAULT_EXPORT_BRAND_ID) {
    return `https://www.dutchdesignersoutlet.com/admin.php?section=products&action=list&filter=brand_id&id=${encodeURIComponent(brandId)}`;
  }

  function fetchDdoExport(brandId) {
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        method: "POST",
        url: exportUrl(brandId),
    ...
    for (const row of rows) {
      ...
      if (!groups.has(key)) {
        groups.set(key, {
          brand,
          productId,
          supplierUrl: `https://chantelle-lingerie.my.site.com/DefaultStore/ccrz__ProductDetails?sku=${encodeURIComponent(productId)}`,
          warehouseId,
  ...
  async function watcherInit() {
    ...
    let products = [];
    api.registerAdapter({
      id: "brands-watcher-chantelle",
      name: "DDO | Brands Watcher - Chantelle & Femilet",
      version: VERSION,
      downloadURL: "https://raw.githubusercontent.com/CPVB86/tempermonkey/main/DDO/watcher/adapters/brands-watcher-chantelle.user.js"
    });
  ...
  function loadProducts(buffer, messagePrefix = "DDO-tabel ingelezen", brand = "Chantelle") {
    ...
    page.addEventListener("brands-watcher:load-ddo", async (event) => {
      try {
        ...
        const brandId = String(event.detail?.brandId || DEFAULT_EXPORT_BRAND_ID);
        api.progress(`DDO-export voor ${brand} ophalen...`);
        loadProducts(await fetchDdoExport(brandId), "DDO-tabel ingelezen", brand);
      } catch (error) {
    ...
    page.addEventListener("brands-watcher:start", async (event) => {
      if (event.detail?.brand && !SUPPORTED_BRANDS.has(event.detail.brand)) return;
      if (running) return;
      running = true;
      paused = false;
      try {
        const chantelleProducts = products.filter((product) =>
          SUPPORTED_BRANDS.has(product.brand) && !["checked", "skipped"].includes(product.checkStatus)
        );

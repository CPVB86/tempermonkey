...
// @updateURL    https://raw.githubusercontent.com/CPVB86/tempermonkey/main/helper/chantelle-card-loader.user.js
// @downloadURL  https://raw.githubusercontent.com/CPVB86/tempermonkey/main/helper/chantelle-card-loader.user.js
// ==/UserScript==
...
(function () {
  ...
  function getMatrixStockValue(info) {
    return String(info?.stockValue ?? info?.stock ?? info?.availableQty ?? info?.qtyAvailable ?? "").trim();
  }

  function isOrderableMatrixInfo(info) {
    if (info?.disabled || info?.isDisabled || info?.unavailable || info?.isUnavailable) return false;
    if (info?.canAddToCart === false || info?.isAddToCartDisabled === true) return false;

    const raw = getMatrixStockValue(info);
  ...
  function buildMatrixItems(stockPayload, baseSku, desiredQtyBySize, fallbackPrice) {
    ...
    function pushItem(outerKey, info) {
      ...
      const desiredQty = desiredQtyBySize.get(sizeKey) || 0;
      const orderable = desiredQty > 0 ? isOrderableMatrixInfo(info) : true;
      const qty = desiredQty > 0 && orderable ? desiredQty : 0;

      if (desiredQty > 0 && orderable) matched.add(sizeKey);
      if (desiredQty > 0 && !orderable) unavailable.add(sizeKey);
    ...
    return {
      ...
      missing: Array.from(desiredQtyBySize.keys()).filter((sizeKey) => !matched.has(sizeKey) && !unavailable.has(sizeKey)),
    };
  }
  ...
  function setRowsState(rows, state, detail = "") {
    rows.forEach((row) => {
      ...
      if (row.statusCell) {
        ...
        row.statusCell.style.color = state === "ok" ? "#16a34a" : state === "error" ? "#dc2626" : "#6b7280";
      }
    });
  }
  ...
  function createPanel() {
    ...
    return { dropButton, orderButton, addRowButton, dryRun, message, tableBody };
  }
  ...
  function renderRows(rows, tableBody) {
    ...
    rows.forEach((row) => {
      ...
      const cells = [
        ...
        { key: "rawSize", value: row.rawSize || row.size },
        { key: "qty", value: row.qty },
      ];
  ...
  function setOrderButtonReady(orderButton, ready) {
    orderButton.disabled = !ready;
    orderButton.style.background = ready ? "#16a34a" : "#9ca3af";
    orderButton.style.cursor = ready ? "pointer" : "not-allowed";
  }
  ...
  function collectRowsFromTable(tableBody) {
    const text = Array.from(tableBody.querySelectorAll("tr"))
      .map((tr) => {
        ...
        const size = tr.querySelector('input[data-field="rawSize"]')?.value || "";
        const qty = tr.querySelector('input[data-field="qty"]')?.value || "";
        return [sku, size, qty].join("\t");
      })
      .join("\n");

    return parseClipboardRows(text);
  }

  function readRawRowsFromTable(tableBody) {
    return Array.from(tableBody.querySelectorAll("tr")).map((tr) => ({
      sku: tr.querySelector('input[data-field="sku"]')?.value || "",
      rawSize: tr.querySelector('input[data-field="rawSize"]')?.value || "",
      size: normalizeSizeKey(tr.querySelector('input[data-field="rawSize"]')?.value || ""),
      qty: tr.querySelector('input[data-field="qty"]')?.value || "",
      state: "idle",
      detail: "",
    }));
  }

  async function dropItemsFromClipboard({ message, tableBody, orderButton }) {
    ...
    if (parsed.errors.length) {
      ...
      setOrderButtonReady(orderButton, false);
      return;
    }
    ...
    message.textContent = `${parsed.rows.length} regel(s) geladen.`;
    setOrderButtonReady(orderButton, true);
  }

  async function runOrder({ dryRun, message, tableBody }) {
    const parsed = collectRowsFromTable(tableBody);

    if (parsed.errors.length) {
      message.textContent = `Fouten in tabel: ${parsed.errors.join(" | ")}`;
      return;
    }

    if (!parsed.rows.length) {
      message.textContent = "Geen regels om te bestellen.";
      return;
    }
    ...
    for (const [sku, sizeEntries] of groups.entries()) {
      ...
      if (matrix.unavailable.length) {
        matrix.unavailable.forEach((size) => {
          const entry = sizeEntries.get(size);
          if (entry) {
            setRowsState(entry.rows, "error", "Niet bestelbaar of geen voorraad");
            entry.rows.forEach((row) => failedRows.add(row));
          }
        });
      }
      ...
      if (!matrix.matched.length) {
        const undecidedRows = groupRowsList.filter((row) => !failedRows.has(row));
        if (undecidedRows.length) {
          setRowsState(undecidedRows, "error", "Geen gevraagde maten gematcht");
          undecidedRows.forEach((row) => failedRows.add(row));
        }
        continue;
      }
  ...
  function init() {
    ...
    ui.dropButton.addEventListener("click", async () => {
      ...
      ui.dropButton.style.opacity = ".65";

      try {
        await dropItemsFromClipboard({ message: ui.message, tableBody: ui.tableBody, orderButton: ui.orderButton });
      } catch (err) {
        console.error(err);
    ...
    ui.orderButton.addEventListener("click", async () => {
      ...
      ui.orderButton.style.opacity = ".65";

      try {
        await runOrder({ dryRun: ui.dryRun.checked, message: ui.message, tableBody: ui.tableBody });
      } catch (err) {
        console.error(err);
        ui.message.textContent = `Fout: ${err?.message || err}`;
      } finally {
        ui.dropButton.disabled = false;
        ui.orderButton.style.opacity = "1";
        setOrderButtonReady(ui.orderButton, ui.tableBody.querySelectorAll("tr").length > 0);
      }
    });

    ui.addRowButton.addEventListener("click", () => {
      const rows = readRawRowsFromTable(ui.tableBody);
      rows.push({ sku: "", size: "", rawSize: "", qty: "", state: "idle", detail: "" });
      renderRows(rows, ui.tableBody);
      ui.message.textContent = "";
      setOrderButtonReady(ui.orderButton, true);
      const lastRow = ui.tableBody.querySelector("tr:last-child");
      lastRow?.querySelector('input[data-field="sku"]')?.focus();
    });

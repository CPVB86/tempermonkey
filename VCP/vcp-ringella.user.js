...
(() => {
  ...
  const CONFIG = {
    ...
    uiDelayMs: 80,
    XLSX_URL: 'https://www.ringella.com/out/downloads/uploads/Ueberhangliste_Sofort_Excel.xlsx',
  };

  const $ = (s, r = document) => r.querySelector(s);
  const delay = (ms) => new Promise(r => setTimeout(r, ms));

  // XLSX data: Map<EAN (string) -> remoteStock (0/1/2)>
  let XLSX_MAP = null;
  ...
  const Logger = {
    ...
    status(id, txt) {
      const sid = String(id);
      if (this._on('console', 'status')) console.info(`[Ringella XLSX][${sid}] status: ${txt}`);
    ...
    perMaat(id, report) {
      ...
      console.groupCollapsed(`[Ringella XLSX][${id}] EAN-vergelijking`);
      try {
    ...
    debug(...a) {
      if (CONFIG.LOG.debug) console.info('[Ringella XLSX][debug]', ...a);
    }
  };
  ...
  // free stock → 0/1/2 zoals eerder besproken
function mapFreeToLevel(free) {
  ...
  function buildXlsxMap(workbook) {
    ...
    // We weten niet zeker welke rij de headers zijn, dus:
    // 1) lees als ruwe array
    // 2) zoek de rij met o.a. "groesse" en "barcode"
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (joined.includes('groesse') && joined.includes('barcode')) {
        headerRowIndex = i;
        break;
      }
    }
    ...
    if (headerRowIndex < 0) {
      console.warn('[Ringella XLSX] geen header rij gevonden (groesse + barcode).');
      return new Map();
    }

    const header = (rows[headerRowIndex] || []).map(h => String(h || '').trim());
    const idxBarcode = header.findIndex(h => h.toLowerCase() === 'barcode');
    const idxFree    = header.findIndex(h => h.toLowerCase() === 'freies_lager');

    if (idxBarcode === -1 || idxFree === -1) {
      console.warn('[Ringella XLSX] geen kolommen "barcode" en/of "freies_Lager" gevonden.');
      return new Map();
    }
    ...
    for (let r = headerRowIndex + 1; r < rows.length; r++) {
      const row = rows[r] || [];
      const rawEan = String(row[idxBarcode] ?? '').trim();
      const eanDigits = rawEan.replace(/\D/g, '');
      if (!eanDigits) continue;

      const freeRaw = row[idxFree];
      const level   = mapFreeToLevel(freeRaw);

      // altijd setten, ook als level 0 is: dan weten we dat hij in de lijst staat maar 0 voorraad heeft
      map.set(eanDigits, level);
    }

    Logger.debug('Ringella XLSX map size:', map.size);
    return map;
  }

  function loadXlsxFromRemote(btn) {
    if (hasXlsxData() || XLSX_LOADING) return;

    XLSX_LOADING = true;
    btn.disabled = true;
    btn.title = 'Ringella XLSX stocklijst wordt geladen...';
      ...
      onload: (resp) => {
        try {
          ...
          btn.title = hasXlsxData()
            ? `Ringella XLSX geladen (${size} EAN-regels).`
            : 'Geen bruikbare EAN-gegevens gevonden in Ringella XLSX.';
          Logger.status('XLSX', `Ringella stocklijst geladen — ${size} EAN-regels`);
        } catch (err) {
          console.error('[Ringella XLSX] fout bij lezen XLSX:', err);
          alert('Kon Ringella XLSX niet lezen. Controleer of de URL nog klopt.');
          XLSX_MAP = null;
          btn.disabled = true;
          btn.title = 'Kon Ringella XLSX niet laden.';
        } finally {
          XLSX_LOADING = false;
        }
      },
      onerror: (err) => {
        console.error('[Ringella XLSX] netwerkfout:', err);
        alert('Kon Ringella XLSX niet ophalen (netwerkfout).');
        XLSX_LOADING = false;
        XLSX_MAP = null;
        btn.disabled = true;
        btn.title = 'Kon Ringella XLSX niet laden.';
  ...
  // Bepaal kolomindices o.b.v. header (zelfde logica als LingaDore)
  ...
  function applyRulesAndMarkFromEAN(localTable, eanMap) {
    ...
    rows.forEach(row => {
      ...
      if (local > 0 && !isAvail) {
        ...
        row.title = `Uitboeken (Ringella XLSX: 0 — EAN ${eanDigits || 'onbekend'})`;
        row.dataset.status = 'remove';
        row.classList.add('status-red');
        actie = 'uitboeken';
      } else if (local === 0 && isAvail) {
        row.style.background = '#d4edda';
        row.title = `Bijboeken 2 (Ringella XLSX >0 — EAN ${eanDigits || 'onbekend'})`;
  ...
  function injectDisabledStyle() {
    if (document.getElementById('ringella-xlsx-disabled-style')) return;
    const style = document.createElement('style');
    style.id = 'ringella-xlsx-disabled-style';
    style.textContent = `
      #check-ringella-xlsx-btn[disabled] {
  ...
  function isRingellaSelected(){
    ...
    return v.includes('ringella') || t.includes('ringella');
  }

  // ---------- Main run ----------
  async function run(btn) {
    if (!hasXlsxData()) {
      alert('Ringella XLSX nog niet geladen. Probeer het zo nog eens.');
      return;
    }
    ...
    for (const table of tables) {
      ...
      try {
        ...
        console.error('[Ringella XLSX] fout:', e);
    ...
    if (CONFIG.LOG.debug) {
      console.info(
        `[Ringella XLSX] verwerkt: ${ok + fail} | geslaagd: ${ok} | fouten: ${fail} | mutaties: ${totalMutations}`
      );
    }
  }
  ...
  // ---------- Toolbar ----------
  function addToolbar() {
    if (document.getElementById('ringella-xlsx-toolbar')) return;

    ensureStockKitCss();
    injectDisabledStyle();

    const bar = document.createElement('div');
    bar.id = 'ringella-xlsx-toolbar';
    bar.style.cssText = `
      position:fixed;
      top:8px;
      right:298px;
    ...
    btn.id = 'check-ringella-xlsx-btn';
    btn.className = 'sk-btn';
    btn.textContent = '🔍 Check Stock Ringella';
    btn.disabled = true;
    btn.title = 'Ringella XLSX wordt geladen...';
    ...
    function toggle() {
      const show = outputHasTables() && isRingellaSelected();
      bar.style.display = show ? 'flex' : 'none';

      if (!show) {
        // bij wissel naar andere leverancier willen we de UI resetten,
        // maar niet per se de XLSX_MAP weggooien (mag je desnoods wél doen)
        btn.disabled = !hasXlsxData();
        btn.title = hasXlsxData()
          ? `Ringella XLSX geladen (${XLSX_MAP.size} EAN-regels).`
          : 'Ringella XLSX wordt geladen...';
        return;
      }

      // Als we moeten tonen en nog geen data hebben: meteen laden
      if (!hasXlsxData() && !XLSX_LOADING) {
        loadXlsxFromRemote(btn);
      } else if (hasXlsxData()) {
        btn.disabled = false;
        btn.title = `Ringella XLSX geladen (${XLSX_MAP.size} EAN-regels).`;
      }
    }

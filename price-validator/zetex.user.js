// ==UserScript==
// @name         DDO | Price Validator - Zetex
// @namespace    https://dutchdesignersoutlet.nl/
// @version      1.1.0
// @description  Valideert geselecteerde of alle zichtbare DDO-producten tegen Zetex en toont remote-status + discount match in de lijst.
// @author       C. P. van Beek
// @match        https://www.dutchdesignersoutlet.com/admin.php?section=products&action=list&filter=brand_id&id=192*
// @match        https://www.dutchdesignersoutlet.com/admin.php?section=products&action=list&filter=brand_id&id=193*
// @match        https://www.dutchdesignersoutlet.com/admin.php?section=products&action=list&filter=brand_id&id=194*
...
// @updateURL    https://raw.githubusercontent.com/CPVB86/tempermonkey/main/price-validator/zetex.user.js
// @downloadURL  https://raw.githubusercontent.com/CPVB86/tempermonkey/main/price-validator/zetex.user.js
// ==/UserScript==
...
(() => {
  ...
  const LOG = '[Zetex List Validator]';

  const HEARTBEAT_KEY = 'zetex_validator_bridge_heartbeat';
  const AUTH_KEY      = 'zetex_validator_bridge_auth';
  const REQ_KEY       = 'zetex_validator_bridge_req';
  const RESP_KEY      = 'zetex_validator_bridge_resp';
  ...
  function findProductByColor(gridJson, colorCode) {
    const wanted = String(colorCode || '').trim().toUpperCase();
    return normalizeProducts(gridJson).find(
      p => String(p?.colorCode || '').trim().toUpperCase() === wanted
    ) || null;
  }
  ...
  if (ON_ZETEX) {
    ...
    async function fetchGrid(styleId, cartId, timeout = TIMEOUT_MS) {
      ...
      if (!webstoreId) throw new Error('Geen webstoreId gevonden in Zetex sessie.');
      if (!effectiveCartId) throw new Error('Geen cartId gevonden in Zetex sessie.');

      const url =
        `https://b2b.zetex.nl/api/shop/webstores/${encodeURIComponent(webstoreId)}` +
  ...
  const BTN_ID = 'zetex-list-validator-btn';
  ...
  function ensureOriginalCellHtml(cell) {
    ...
    if (!cell.dataset.zetexOriginalHtml) {
      cell.dataset.zetexOriginalHtml = cell.innerHTML;
    }
    return cell.dataset.zetexOriginalHtml;
  }
  ...
  function setPublicResult(row, state) {
    ...
    appendBadge(cell, html, 'zetex-public-badge');
  }
  ...
  async function validateRow(row) {
    ...
    try {
      ...
      row.dataset.zetexProductId = productId;
      row.dataset.zetexSupplierPid = meta.supplierPid;
      row.dataset.zetexStyleId = pidParts.styleId;
      row.dataset.zetexColorCode = pidParts.colorCode;
  ...
  async function runBatchValidation() {
    if (!bridgeOnline()) {
      alert(
        ...
        'Open eerst een b2b.zetex.nl-tab met dit script actief,\n' +

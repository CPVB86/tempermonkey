...
// @updateURL    https://raw.githubusercontent.com/CPVB86/tempermonkey/main/SC4/stock-check-rj-bodywear.user.js
// @downloadURL  https://raw.githubusercontent.com/CPVB86/tempermonkey/main/SC4/stock-check-rj-bodywear.user.js
// ==/UserScript==
...
(() => {
  ...
  // -----------------------
  if (!Core) {
    console.error('[Stock Check|RJ Bodywear] VCPCore ontbreekt.');
    return;
  }
  if (!SR || typeof SR.mapRemoteToTarget !== 'function' || typeof SR.reconcile !== 'function') {
    console.error('[Stock Check|RJ Bodywear] StockRules ontbreekt of is incompleet.');
    return;
  }
  ...
  function normalizeSize(s) {
    ...
    // Extern XXL en intern 2XL zijn equivalent.
    if (out === 'XXL') out = '2XL';
    return out;
  }
  ...
  async function buildRjIndexOnce() {
    ...
    RJ_INDEX_PROMISE = (async () => {
      ...
      for (const path of paths) {
        for (let batchStart = 1; batchStart <= MAX_PAGES; batchStart += INDEX_CONCURRENCY) {
          const pages = Array.from(
            { length: Math.min(INDEX_CONCURRENCY, MAX_PAGES - batchStart + 1) },
          ...
          const results = await Promise.all(pages.map(async page => {
            ...
            catch { return { page, empty: false }; }

            if (!(res.status >= 200 && res.status < 300) || isNotFoundHttp(res.status)) {
              return { page, empty: false };
            }

            const configs = extractAllJsonConfigsFromHtml(res.text);
            if (!configs.length) return { page, empty: true };

            for (const cfg of configs) addConfigToIndex(index, cfg, { path, page, url });
            return { page, empty: false };
          }));

          if (results.some(result => result.empty)) break;
        }
      }
  ...
  // =========================
  // Vergelijk via StockRules en markeer via Core.
  // =========================
  ...
  async function run(btn) {
    ...
    btn.dataset.skState = 'busy';
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
    btn.title = 'RJ Bodywear-index opbouwen';

    let index;
    try {
      index = await buildRjIndexOnce();
    } catch (e) {
      console.error('[Stock Check|RJ Bodywear] index build error:', e);
      ...
      btn.title = 'Controleer voorraad bij RJ Bodywear';
    }
  ...
  const { btn } = Core.mountSupplierButton({
    id: 'stock-check-rj-bodywear-btn',
    text: 'Controleer RJ Bodywear',
    right: 250,
    top: 8,
    match: () => isRjSelected(),
    onClick: (btn) => run(btn),
  ...
  btn.innerHTML = '<i class="fa-solid fa-magnifying-glass-chart"></i>';
  btn.setAttribute('aria-label', 'Controleer voorraad bij RJ Bodywear');
  btn.title = 'Controleer voorraad bij RJ Bodywear';

...
// @updateURL    https://raw.githubusercontent.com/CPVB86/tempermonkey/main/SC4/stock-check-charlie-choe.user.js
// @downloadURL  https://raw.githubusercontent.com/CPVB86/tempermonkey/main/SC4/stock-check-charlie-choe.user.js
// ==/UserScript==
...
(() => {
  ...
  if (!Core) {
    console.error('[Stock Check|Charlie Choe] VCPCore ontbreekt.');
    return;
  }
  if (!SR || typeof SR.mapRemoteToTarget !== 'function' || typeof SR.reconcile !== 'function') {
    console.error('[Stock Check|Charlie Choe] StockRules ontbreekt of is incompleet.');
    return;
  }
  ...
  // ---------- Per-table ----------
  function applyRulesAndMark(table, remoteMap, anchorId){
  ...
  async function perTable(table){
    ...
    try {
      ...
      return applyRulesAndMark(table, remote, anchorId);
    } catch (e) {
      console.error('[Stock Check|Charlie Choe] fout:', e);
  ...
  const { btn } = Core.mountSupplierButton({
    id:   'stock-check-charlie-choe-btn',
    text: 'Controleer Charlie Choe',
    right: 250,
    top: 8,
  ...
  btn.setAttribute('aria-label', 'Controleer voorraad bij Charlie Choe');
  btn.title = 'Controleer voorraad bij Charlie Choe';

...
// @updateURL    https://raw.githubusercontent.com/CPVB86/tempermonkey/main/SC4/stock-check-wacoal.user.js
// @downloadURL  https://raw.githubusercontent.com/CPVB86/tempermonkey/main/SC4/stock-check-wacoal.user.js
// ==/UserScript==
...
(() => {
  ...
  const btn = document.querySelector('#stock-check-wacoal-recheck-btn');
  const exportBtn = document.querySelector('#stock-check-wacoal-export-btn');
  ...
  if (!isRunning) {
    btn.title = `${count} onzeker(e) artikel(en) opnieuw controleren`;
    btn.setAttribute('aria-label', btn.title);
    exportBtn.title = `${count} onzeker(e) artikel(en) exporteren naar XLSX`;
    exportBtn.setAttribute('aria-label', exportBtn.title);
  }

  btn.classList.toggle('vcp-show', count > 0);
  exportBtn.classList.toggle('vcp-show', count > 0);
}
...
function exportCheckenXlsx() {
  ...
  console.info(`[Stock Check | Wacoal] ${rows.length} checken-regel(s) geexporteerd.`);
}
  ...
  const mounted = Core.mountSupplierButton({
    id: 'stock-check-wacoal-btn',
    text: 'Controleer Wacoal Group',
    right: 250,
    top: 8,
  ...
  const recheckMounted = Core.mountSupplierButton({
    id: 'stock-check-wacoal-recheck-btn',
    text: 'Hercontroleer onzekere artikelen',
    right: 470,
    top: 8,
    match: () => isSupportedSelected(),
    onClick: btn => rerunChecken(btn)
  });
  recheckMounted.btn.innerHTML = '<i class="fa-solid fa-rotate"></i>';
  recheckMounted.btn.setAttribute('aria-label', 'Onzekere Wacoal-artikelen opnieuw controleren');
  recheckMounted.btn.title = 'Onzekere Wacoal-artikelen opnieuw controleren';

  const exportMounted = Core.mountSupplierButton({
    id: 'stock-check-wacoal-export-btn',
    text: 'Exporteer onzekere artikelen',
    right: 650,
    top: 8,
    match: () => isSupportedSelected(),
    onClick: () => exportCheckenXlsx()
  });
  exportMounted.btn.innerHTML = '<i class="fa-solid fa-file-excel"></i>';
  exportMounted.btn.setAttribute('aria-label', 'Exporteer onzekere Wacoal-artikelen');
  exportMounted.btn.title = 'Exporteer onzekere Wacoal-artikelen';

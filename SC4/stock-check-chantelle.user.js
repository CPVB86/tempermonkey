...
function normalizeSizeKey(raw) {
  ...
  // Alleen alternatieven met | of , afkappen
  // Maar NIET slash-maten zoals XS/S, M/L, XL/2XL
  v = v.split(/[|,]/)[0];
  ...
  if (/^(XXXS|XXS|XS|S|M|L|XL|2XL|3XL|4XL|5XL|6XL|TU)$/.test(v)) return v;
...
function isSizeLabel(s) {
  ...
  function isChantelleSelected() {
    ...
    return /^(chantelle|femilet)(?:\s|$)/.test(byValue)
      || /\b(chantelle|femilet)\b/.test(byText);
  }
  ...
  if (ON_TOOL) {
    ...
    mounted.btn.setAttribute('aria-label', 'Controleer voorraad bij Chantelle of Femilet');
    mounted.btn.title = 'Controleer voorraad bij Chantelle of Femilet';
